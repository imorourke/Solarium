...
def main():
    ...
    _ = p.add_argument("--no-cargo", action="store_true", help="skip running the cargo command to update the lock file")
    args = p.parse_args()
    ...
    files_to_check: list[tuple[Path, str, str]] = [
        (base_path / "doc" / "isa.tex", r"\\date{(?P<current>v[\d\w\.]+)\s+(?P<rest>[^\s].*)}", rf"\\date{{v{version_str} \g<rest>}}"),
        (base_path / "Cargo.toml", r"\[workspace.package\]\nversion = \"[\d\.]+\"", f'[workspace.package]\nversion = "{version_str}"'),
        (base_path / "CMakeLists.txt", r"project\(SolariumProcessor VERSION (?P<current>[\d\.]+)\)", f"project(SolariumProcessor VERSION {version_str})"),
    ]
    ...
    if should_run_cargo:
        p = subprocess.Popen(["cargo", "update"], stdin=subprocess.PIPE, stderr=subprocess.PIPE, cwd=base_path)
        _ = p.communicate()
        if p.returncode != 0:
